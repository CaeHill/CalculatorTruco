...
function main() {
  ...
  if (hand.length && !deckChoose) {
    // If the user opened calculator.html directly without choosing a deck,
    // avoid requesting invalid image paths like img/null/0.png.
    window.location.href = "deckChooser.html";
    return;
  }
...
function printResult(resultCardsVet) {
  ...
  const deckChoose = sessionStorage.getItem("deckChoose");
  if (!deckChoose) {
    // Avoid broken image requests if no deck has been selected.
    return;
  }
  ...
  for (i = 0; i < reverseVet.length; i++) {
    ...
    //Specific treatment for the Point image
    if (card == ".") {
      resultSlot.style.backgroundImage =
        "url('/img/" + deckChoose + "/point.png')";
    }
